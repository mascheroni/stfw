...
import com.almundo.automation.entities.*;
...
public class FlightSearchRegressionTest {

	private ResponseData responseData;
	private Map<String, String> data;

	public FlightSearchRegressionTest(ResponseData responseData, Map<String, String> data) {
		this.responseData = responseData;
		this.data = data;
	}

	@Test(description = "Verify common data", groups = "flight-search-regression")
	public void verifyCommonData() {
		...
		// Esto depende del set de datos, no puede estar asi nomas
		for (Cluster cluster : results.getClusters()) {
			...
			if (cluster.isDomestic()) {
				Assert.assertEquals(cluster.getPrice().getDetail()
								.getExtraTax(), 0,
						"The itinerary with validating carrier " + cluster.getValidatingCarrier()
								+ " has extra tax: "
								+ cluster.getPrice().getDetail().getExtraTax());
			} else {
				Assert.assertTrue(cluster.getPrice().getDetail()
								.getExtraTax() > 0,
						"The itinerary with validating carrier " + cluster.getValidatingCarrier()
								+ " does not have extra tax");
			}
